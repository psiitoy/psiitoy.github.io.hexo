...
a) 中心化(统一入口)
b) 双写(先同步写mysql如果发生异常改异步，尽量避免服务不可用)
c) 倒库(jproxy支持通过游标形式全量遍历库-逐个表操作，可以利用其异步同步数据)
d) 数据校验
e) 切库提供服务
...
sharding-jdbc(规则模块+数据源管理模块)，开发量减少95%。
