...
node->treeNode(jdk8) 当某一个点上的元素数量打到一定的阈值的时候，链表会变成一颗树，这样在极端情况下(所有的元素都在一个点上，整个就以链表)，一些操作的时间复杂度有O(n)变成了O(logn)。
...
4) 空间大小
栈 通过-Xss配置，如果递归没有及时跳出，很可能发生StackOverFlowError问题。
堆 通过-Xms、-Xmx配置

> 参见[Java中的堆和栈的区别](https://link.zhihu.com/?target=http%3A//droidyue.com/blog/2014/12/07/differences-between-stack-and-heap-in-java/)
